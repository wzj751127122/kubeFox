	...
	return func(c *gin.Context) {
		...
		if c.Request.Method != http.MethodGet {
			...
			if len(m) > 0 {
				...
				if err != nil {
					zap.L().Error("marshal body error:", zap.Error(err))
					body = []byte{}
				}
			}
		}
		...
		if err != nil {
			zap.L().Error("get claims from token err:", zap.Error(err))
			return
		}
		userId = claims.ID
		...
		if err := logic.CreateOperationRecord(c, &record); err != nil {
			zap.L().Error("create operation record error:", zap.Error(err))
		}
	}
}
